...
from vanna.core._compat import StrEnum
...
class UiFeature(StrEnum):
    ...
    UI_FEATURE_SECURITY_SETTINGS = "security_settings"
...
DEFAULT_UI_FEATURES: Dict[str, List[str]] = {
    ...
    UiFeature.UI_FEATURE_SECURITY_SETTINGS: ["admin"],
}
...
class SecurityConfig(BaseModel):
    ...
    These settings can be toggled by admins via the Security settings
    tab in the admin UI.  Defaults are secure-by-default.
    """

    ssrf_protection_enabled: bool = Field(
        default=True,
        description="Block requests to private/internal IP addresses from fetch_url",
    )
    ssrf_allowed_private_ranges: List[str] = Field(
        default_factory=list,
        description="Admin-whitelisted CIDR ranges (e.g. '10.0.0.0/8') allowed even when SSRF protection is on",
    )
    max_redirect_hops: int = Field(
        default=5,
        ge=0,
        le=10,
        description="Maximum number of HTTP redirects to follow in fetch_url",
    )
    fetch_url_enabled: bool = Field(
        default=True,
        description="Master switch for the fetch_url tool",
    )
    lineage_isolation_enabled: bool = Field(
        default=True,
        description="Store lineage evidence per-conversation (vs process-global)",
    )
    lineage_max_entries: int = Field(
        default=100,
        ge=10,
        le=10_000,
        description="Maximum number of conversation lineage entries to retain (LRU)",
    )
    api_strict_validation: bool = Field(
        default=True,
        description="Reject unexpected fields in public API request models",
    )
...
class AgentConfig(BaseModel):
    ...
    max_tool_iterations: int = Field(default=25, gt=0)
